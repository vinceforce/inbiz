...
$(document).ready(function () {
    ...
    $('.actCreateMarque').on('click', function () {
        document.location.href = '/marques/mask/';
    });
    ...
    if (oMarqueDetail.length) {
        oMarqueDetail.find('input[type=text],select').each(function () {
            $(this).prop('disabled', true);
        });
        oMarqueDetail.find('.styled-select').each(function () {
            $(this).addClass('disabled').css({'background':'none'});
        });
    }
    ...
    if (oMarqueDetail.length) {
        ...
        oMarqueDetail.find('.marquedatas').find('[disabled]').prop('disabled', false);
    ...
    $('.page-footer-wrapper .actvalider').on('click', function () {
        ...
        if (oMarque.length) {
            ...
            $.ajax({
                    method: $('.marquedetail form').attr('method'),
                ...
                .done(function (msg) {
                    ...
                    if (bModeCreation) {
                        ...
                        console.log('avant parsing json --> ' + msg)
                        // var oReturn = jQuery.parseJSON(msg);
                        console.log('jReturn.marqueId ' + msg.marqueId);
                        //ajout des ID de marque
                        $('section.marquecontact .contactitem input[data-name=idmarque]').val(msg.marqueId);
                        $('.pagetitle').html('DÉTAILS DE ' + msg.marqueName)
                        fnSendAllContact();
                        self.location.href = '/marques/mask/' + msg.marqueId;
                    }
                })
                .fail(function (e) {
                    console.log(e);
...
function fnSendAllContact() {
    ...
    if (oContactForms.length) {
        if (oContactForms.length > contactSendIndex) {
            ...
            oContactItem.removeClass('submitatt submitok submitfail').addClass('submitatt');
            $.ajax({
                    method: oContactItem.find('form').attr('method'),
                    url: oContactItem.find('form').attr('action'),
                    data: oContactItem.find('form').serialize()
                })
                .done(function (msg) {
